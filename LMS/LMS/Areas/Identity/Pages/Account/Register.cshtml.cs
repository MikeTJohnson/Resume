...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        /// <summary>
        /// Creates a unique uID for a new user
        /// </summary>
        /// <returns>A string containing the new unique uID</returns>
        string UniqueUid(string role) {
            // Query the database to find the max uID for each role
            int uid_num;
            if (role == "Administrator") {
                var query = (from u in db.Administrators
                             select u.UId).DefaultIfEmpty().Max();
                if (query is not null) {
                    string uid_max = string.Concat(query);
                    Console.WriteLine(uid_max);
                    string num = uid_max.Substring(1, 7);
                    uid_num = Int32.Parse(num);
                    uid_num += 1;
                    if (uid_num > 1999999) {
                        return "null";
                    }
                ...
                var query = (from u in db.Professors
                             select u.UId).DefaultIfEmpty().Max();
                if (query is not null)
                {
                    string num = query.Substring(1, 7);
                    uid_num = Int32.Parse(num);
                    uid_num += 1;
                    if (uid_num > 3999999) {
                        return "null";
