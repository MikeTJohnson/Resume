    ...
    public class CommonController : Controller
    {
        ...
        public IActionResult GetClassOfferings(string subject, int number)
        {
            var query = (from c in db.Classes
                         where c.CIdNavigation.DAbr == subject
                         && c.CIdNavigation.Num == number
                         select new
                         {
                             season = c.Season,
                             year = c.Year,
                             ...
                             fname = c.ProfNavigation.FName,
                             lname = c.ProfNavigation.LName
                         }).ToArray();
            return Json(query);
        }
        ...
        var query = (from s in db.Submissions
                         ...
                         select s.SubmitContents).SingleOrDefault();
            string answer = "";
            answer += query;
            return Content(answer);
        }
        ...
        public IActionResult GetUser(string uid)
        {
            if (User.IsInRole("Administrator"))
            {
                ...
                // Return the user information if they exist
                ...
                // Return the user information if they exist
                if (studQuery is not null)
                {
                    return Json(studQuery);
                }
            }
            // If the user information returned null, then send failure message
