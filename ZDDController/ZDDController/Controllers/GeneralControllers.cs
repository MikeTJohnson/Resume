	...
	public class GeneralControllers
	{
        ...
        public bool startSort(int eID, System.DateTime startTime, int fo, String pNum)
		{
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                string sidQuery = "select sID from sorts natural join parts where pNum = @pNum and FO = @fo";
                string insertQuery = "insert into work (eID, sID, startTime) " +
                    "VALUES (@eID, @sID, @startTime)";
                int sID = 0;
                ...
                try
                {
                    ...
                    //get the sID
                    using (MySqlCommand command = new MySqlCommand(sidQuery, connection, transaction))
                    {
                        command.Parameters.AddWithValue("@pNum", pNum);
                        command.Parameters.AddWithValue("@fo", fo);
                    ...
                    //insert the eID, sID, and start time into the work table
                    //throws an error if the FO and part number don't match
